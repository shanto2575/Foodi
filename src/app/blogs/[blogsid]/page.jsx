...
const BlogsDetails = async ({ params }) => {
    ...
    const blog = blogs.find(blog => blog.id === Number(blogsid))

    const { id, title, author, date, tags, rating, image, excerpt,category } = blog;

    // console.log(blog)
    return (
        <div>
            <div className="card lg:card-side flex gap-10 bg-base-100 my-10 p-4">
                <figure className="card h-[450px] w-[400px] rounded-2xl">
                    <Image
                        src={image}
                        alt={title}
                        fill
                ...
                <div className="card space-y-5">
                    ...
                    <div>
                        <ul className="flex gap-5 items-center  ">
                            {tags.map((tag, index) =>
                                <li key={index} className="badge badge-info flex gap-5 my-3">{tag}</li>)}
                        </ul>
                    </div>
                    <p>Excerpt : {excerpt}</p>
                    <p>Date : {date}</p>
